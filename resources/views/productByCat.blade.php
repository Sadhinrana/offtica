                    ...
                    <aside class="widget widget-banner hidden-sm">
                        <div class="widget-info widget-banner-img">
                            @foreach($banners as $banner)
                                <a href="{{$banner->banner_link_category_page}}"><img src="{{asset('storage/images/banner/'.$banner->banner_category_page)}}" alt="" /></a>
                            @endforeach
                        </div>
                    </aside>
                    <!-- Widget-banner end -->
                    ...
                    <div class="shop-content mt-tab-30 mt-xs-30">
                        ...
                        <div class="tab-content">
                            <div class="tab-pane active" id="grid-view">
                                ...
                                @foreach($products as $product)
                                    <!-- Single-product start -->
                                        <div class="col-lg-4 col-md-4 col-sm-6 col-xs-12 product @foreach($product->colors as $color){{$color->color}} @endforeach @foreach($product->sizes as $size){{$size->size}} @endforeach" data-price="{{$product->salePrice}}">
                            ...
                            <div class="tab-pane" id="list-view">
                                ...
                                @foreach($products as $product)
                                    <!-- Single-product start -->
                                        <div class="col-lg-12 productList c{{$product->category_id}} s{{$product->subcategory_id}} m{{$product->minicategory_id}} @foreach($product->colors as $color){{$color->color}} @endforeach @foreach($product->sizes as $size){{$size->size}} @endforeach" data-price="{{$product->salePrice}}">
...
@section('script')
    <script>
        ...
        @foreach($categories as $category)
            ...
            href:"{{url('/productByCat/'.$category->id)}}",
            nodes: [
            @foreach($category->subcategory as $subcategory)
                {
                text: '{{$subcategory->subCategoryName}}',
                href:"{{url('/productByCat/'.$category->id."/".$subcategory->id)}}",
                nodes: [
                @foreach($subcategory->minicategory as $minicategory)
                    {
                    text: '{{$minicategory->miniCategoryName}}',
                    href:"{{url('/productByCat/'.$category->id."/".$subcategory->id."/".$minicategory->id)}}"
                    },
                @endforeach
                ]
                },
